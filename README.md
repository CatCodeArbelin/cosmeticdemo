# DEMO-прототип помощника менеджера косметического магазина

Это демонстрационный проект (не production), который показывает механику:

**FakeMAX / Telegram → Telegram-хаб менеджера → варианты ответов → выбор ответа → “отправка” клиенту**.

MAX имитируется через:
- команду `/demo_max`
...
3. Временный способ: добавьте debug-лог `message.chat.id` в обработчик.
